...
class LoginScreenAnimation {
	...
	constructor(canvas: HTMLCanvasElement | null) {
		...
		let i;
		for (i = 0; i < 64; ++i)
			this.field1220[i] = i * 0x40000;
		for (i = 0; i < 64; ++i)
			this.field1220[i + 64] = i * 0x400 + 0xFF0000;
		for (i = 0; i < 64; ++i)
			this.field1220[i + 128] = i * 4 + 0xFFFF00;
		for (i = 0; i < 64; ++i)
			this.field1220[i + 192] = 0xFFFFFF;
		for (i = 0; i < 64; ++i)
			this.field1217[i] = i * 0x400;
		for (i = 0; i < 64; ++i)
			this.field1217[i + 64] = i * 4 + 0x00FF00;
		for (i = 0; i < 64; ++i)
			this.field1217[i + 128] = i * 0x40000 + 0xFFFF;
		for (i = 0; i < 64; ++i)
			this.field1217[i + 192] = 0xFFFFFF;
		for (i = 0; i < 64; ++i)
			this.field1222[i] = i * 4;
		for (i = 0; i < 64; ++i)
			this.field1222[i + 64] = i * 0x40000 + 0xFF;
		for (i = 0; i < 64; ++i)
			this.field1222[i + 128] = i * 0x400 + 0xFF00FF;
		for (i = 0; i < 64; ++i)
			this.field1222[i + 192] = 0xFFFFFF;
	...
	method2207(var1: number, cycle: number) {
		...
		if (this.field1229 > this.field1233.length) {
			...
			if (Sprite.CACHE != null)
				this.method2215(Sprite.CACHE[Math.floor(Math.random() * Sprite.CACHE.length)]);
		}
		...
		for (let var5 = 0; var5 < var4; ++var5) {
			var6 = this.field1225[var3 + var2] - Math.floor(
				this.field1233[var2 + this.field1229 & this.field1233.length - 1] * var1 / 6
			);
			if (var6 < 0)
				var6 = 0;
		...
		if (this.field1235 === 0 && this.field1224 === 0) {
			...
			if (var7 === 0)
				this.field1235 = 0x400;
			if (var7 === 1)
				this.field1224 = 0x400;
		}
		...
		if (var7 > 0) {
			...
			for (var11 = 0; var11 < 256; ++var11) {
				...
				for (var14 = -var7; var14 < 128; ++var14) {
					...
					if (var14 >= 0)
						this.field1214[var13 + var14] = Math.floor(var12 / (Math.imul(var7, 2) + 1));
				}
			}
	...
	method2215(sprite: Sprite | undefined) {
		let i;
		for (i = 0; i < this.field1233.length; ++i)
			this.field1233[i] = 0;
